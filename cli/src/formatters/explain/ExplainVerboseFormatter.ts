...
export class ExplainVerboseFormatter implements Formatter {
  ...
  showExplanation(explanation: ExecutionExplanation): void {
    const headerDivider = chalk.cyan(divider(80, '═'));
    ...
    if (explanation.adaptersUsed && Array.isArray(explanation.adaptersUsed) && explanation.adaptersUsed.length > 0) {
      this.logger.info(chalk.magentaBright(`▶ Adapters Used: ${explanation.adaptersUsed.join(', ')}`));
    }
    if (explanation.tags && Array.isArray(explanation.tags) && explanation.tags.length > 0) {
    ...
    if (explanation.inputs && Object.keys(explanation.inputs).length > 0) {
      ...
      this.logger.info(chalk.gray('  No inputs defined.'));
    }

    // Steps
    this.logger.info(chalk.bold('\nExecution Plan:'));
    if (explanation.steps && Array.isArray(explanation.steps) && explanation.steps.length > 0) {
      explanation.steps.forEach((step, i) => {
        ...
        if (step.needs && Array.isArray(step.needs) && step.needs.length > 0) {
        ...
        if (step.with && typeof step.with === 'object' && Object.keys(step.with).length > 0) {
    ...
    if (explanation.outputs && typeof explanation.outputs === 'object' && Object.keys(explanation.outputs).length > 0) {
